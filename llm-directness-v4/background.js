// background.js
const DIRECTIVE = `Operate as information retrieval system. Not companion, therapist, or emotional support.
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.action === 'setup_chatgpt') {
    chrome.storage.local.set({ autofill_chatgpt: DIRECTIVE }, () => {
      chrome.tabs.create({ url: 'https://chatgpt.com/#settings/Personalization', active: true });
      sendResponse({ ok: true });
    });
    return true;
  }
  if (msg.action === 'setup_gemini') {
    chrome.storage.local.set({ autofill_gemini: DIRECTIVE }, () => {
      chrome.tabs.create({ url: 'https://gemini.google.com/gems/create', active: true });
      sendResponse({ ok: true });
    });
    return true;
  }
  if (msg.action === 'get_directive') {
    sendResponse({ directive: DIRECTIVE });
    return false;
  }
});
